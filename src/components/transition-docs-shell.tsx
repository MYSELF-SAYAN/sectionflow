    ...
    <div className="border-b border-zinc-800 bg-[#0a0d13]">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-3">
        <div className="flex items-center gap-2 text-xs text-zinc-400">
          <Terminal className="size-3.5 text-teal-400 shrink-0" />
      ...
      <div className="flex items-center gap-2 border-t border-zinc-800/60 bg-[#07090f] px-4 py-3">
        ...
        <code className="flex-1 font-mono text-sm text-teal-300">{command}</code>
        <button
          type="button"
          onClick={copy}
...
export function TransitionDocsShell({
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#101014] p-2.5 sm:p-3 lg:p-4">
        {/* Header */}
        <div className="rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(45,212,191,0.16),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-4 sm:p-5 lg:p-6">
            ...
            {useCases.map((item) => {
              ...
              return (
                <div key={item.label} className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="flex items-center gap-2 text-teal-300"><Icon className="size-4" /><span className="text-sm font-semibold text-white">{item.label}</span></div>
        ...
        {/* Preview / Code panel */}
        <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20 shadow-[0_0_80px_rgba(45,212,191,0.12)]">
          {/* Toolbar */}
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-[linear-gradient(90deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] px-4 py-3 text-[11px] uppercase tracking-[0.32em] text-zinc-500">
            ...
            <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/40 p-1">
              <button type="button" onClick={() => setActiveTab('demo')} className={`rounded-full px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.25em] transition ${activeTab === 'demo' ? 'bg-teal-500 text-teal-950' : 'text-zinc-400 hover:text-white'}`}>Preview</button>
              <button type="button" onClick={() => setActiveTab('code')} className={`rounded-full px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.25em] transition ${activeTab === 'code' ? 'bg-teal-500 text-teal-950' : 'text-zinc-400 hover:text-white'}`}>Code</button>
            </div>
            <div className="flex items-center gap-2">
              <Tooltip label={copiedPrompt ? 'Copied!' : 'Copy AI prompt'}>
                <button type="button" onClick={() => copyToClipboard(aiPrompt, 'prompt')} className="rounded-full border border-white/10 bg-white/5 p-2 text-zinc-400 transition hover:bg-white/10 hover:text-teal-300" aria-label="Copy AI prompt">
              ...
              <Tooltip label={copiedAll ? 'Copied!' : 'Copy all files'}>
                <button type="button" onClick={() => copyToClipboard(allCode, 'all')} className="rounded-full border border-white/10 bg-white/5 p-2 text-zinc-400 transition hover:bg-white/10 hover:text-white" aria-label="Copy all source files">
                  {copiedAll ? <Check className="size-4 text-emerald-400" /> : <Copy className="size-4" />}
                </button>
              </Tooltip>
              <Tooltip label="Full-screen demo">
                <Link href={`/demo/${transition.slug}`} className="rounded-full border border-white/10 bg-white/5 p-2 text-zinc-400 transition hover:bg-white/10 hover:text-white" aria-label="Open full-screen demo">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <aside className="space-y-6">
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <button type="button" onClick={() => copyToClipboard(aiPrompt, 'prompt')} className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-white/10 hover:text-white">
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-4 space-y-3">
              {relatedTransitions.map((item) => (
                <Link key={item.slug} href={`/docs/transitions/${item.slug}`} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-zinc-400 transition hover:bg-white/10 hover:text-white">
                  <span>{item.name}</span>
