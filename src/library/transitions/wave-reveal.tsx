...
export interface WaveRevealProps extends SectionTransitionProps {
  /** Color class for the wave cap – should match the second section's background. */
...
function Inner({ first, second, waveClassName }: Omit<WaveRevealProps, 'height' | 'className'>) {
  ...
  const y = useTransform(p, [0.30, 0.95], ['102%', '0%']);
  const dim = useTransform(p, [0.30, 0.95], [1, 0.55]);
  const waveScale = useTransform(p, [0.30, 0.62, 0.95], [1, 1.6, 0.0001]);

  return (
    <>
      <motion.div style={{ opacity: dim }} className="absolute inset-0">{first}</motion.div>
      <motion.div style={{ y }} className="absolute inset-0">
        <motion.div style={{ scaleY: waveScale }} className={`absolute top-[-12vh] left-0 h-[12.5vh] w-full origin-bottom ${waveClassName ?? 'text-[#0e0e11]'}`}>
...
/** WaveReveal – an SVG wave cap rides the incoming section as it sweeps upward. */
