...
function Inner({ first, second }: Pick<SectionTransitionProps, 'first' | 'second'>) {
  ...
  // Dead zone: 0→0.30 — first section fully visible, glass panel parked below screen
  const panelY = useTransform(p, [0.30, 0.92], ['100%', '0%']);
  const glassPanelOpacity = useTransform(p, [0.30, 0.48, 0.82, 0.94], [0, 1, 1, 0]);
  const firstDim = useTransform(p, [0.30, 0.90], [1, 0]);
  const secondReveal = useTransform(p, [0.68, 0.96], [0, 1]);
  ...
  return (
    <>
      ...
      <motion.div
        style={{ y: panelY, opacity: glassPanelOpacity }}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  WebkitBackdropFilter: 'blur(8px) brightness(1.04)',
                  borderTop: i % 3 === 0 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                }}
              />
            );
          })}
...
/** GlassDistortion – a refractive frosted-glass panel slides up and dissolves to reveal the next section. */
