...
export function Shatter({ first, second, height = 350, className }: SectionTransitionProps) {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(root.current!.querySelector('.sf-shatter-second'), { autoAlpha: 0 }, { autoAlpha: 1, ease: 'none', duration: 0.18 }, 0.30);
      shards.forEach((shard, i) => {
        const { tx, ty, rot } = TRAJECTORIES[i];
        const delay = 0.25 + (i / shards.length) * 0.30;
        tl.fromTo(shard, { x: 0, y: 0, rotation: 0, scale: 1, opacity: 1 }, { x: tx, y: ty, rotation: rot, scale: 0.4, opacity: 0, ease: 'power2.in', duration: 0.45 }, delay);
      });
    }, root);
    return () => ctx.revert();
  }, []);
