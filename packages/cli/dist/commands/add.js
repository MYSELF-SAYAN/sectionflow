...
import { TYPES_SOURCE, SECTION_FLOW_SOURCE, REGISTRY_SKELETON, TIMING_OVERRIDES, } from '../core-files.js';
...
function ensureCore(config) {
    ...
    const missing = !fs.existsSync(typesPath) ||
    ...
    if (!fs.existsSync(sectionFlowPath))
        fs.writeFileSync(sectionFlowPath, SECTION_FLOW_SOURCE, 'utf8');
    if (!fs.existsSync(registryPath))
        fs.writeFileSync(registryPath, REGISTRY_SKELETON, 'utf8');
 ...
 */
function appendToRegistry(registryPath, slug) {
    ...
    if (timing) {
        ...
        if (!result.includes(TIMING_IMPORT)) {
            result = result.replace(/^(import type \{ TransitionResolver \} from '\.\/types';)/m, `$1\n${TIMING_IMPORT}`);
        }
